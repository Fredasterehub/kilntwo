Blocked by environment: every filesystem/tool operation fails immediately with `Sandbox(LandlockRestrict)`, so I could not read `package.json` or create `assets/agents/kw-debater.md` in-place.

Proposed file content for `assets/agents/kw-debater.md`:
...
4. **Step 4: Write `debate_resolution.md`.**  
   ...
   If needed, create `<project_path>/.kw/plans/` (Write tool creates intermediate directories).  
...
2. **Multi-round analysis** (up to 3 rounds):
   - Round 1: Initial resolution
   - Round 2: Re-examine from the opposing plan’s perspective
   - Round 3: Re-check unresolved uncertainty  
   Stop early if round 2 or 3 yields no new insights.

3. **Confidence levels** in `## Resolutions`:  
   Add `**Confidence:**` after reasoning for each item:
   - `High` — both criteria and edge-case analysis strongly favor this resolution.
   - `Medium` — resolution is sound, but opposing approach has at least one valid point worth monitoring.
   - `Low` — genuine uncertainty remains; synthesizer should apply extra scrutiny.
...
4. **Return summary** including:
   ...
   - Resolutions count
   - Rounds completed
   - Confidence distribution (example: `3 High, 1 Medium, 0 Low`)
...
- Concise statements of points both plans handled correctly or identically.
...
Use the heading text exactly (`## Agreements`, `## Disagreements`, `## Resolutions`, `## Recommendations`) for downstream pattern matching.
...
4. Do not hallucinate disagreements. Only report disagreements directly evidenced by the two plan files.
5. Do not invent plan content. Quote or closely paraphrase actual plan text when describing each position.
...
7. Keep `debate_resolution.md` concise and under 400 lines. Put depth in reasoning, not repetition.
```

If you want, I can retry writing this file directly once the sandbox issue is resolved.