I couldn’t write to the workspace in this session because all tool executions are failing with `Sandbox(LandlockRestrict)`, so I couldn’t create the file directly or run `node -e` verification.

Use this exact `src/uninstall.js`:
