I could not write or run commands in this session because every tool call failed with a sandbox error (`LandlockRestrict`).  
Below is the exact implementation for the two files per your spec.
...
function resolvePaths(homeOverride) {
  ...
  const configDir = path.join(home, '.claude', 'kilntwo');
  const manifestPath = path.join(configDir, 'manifest.json');

  return { home, configDir, manifestPath };
}
...
function readManifest(homeOverride) {
  const { manifestPath } = resolvePaths(homeOverride);
...
function writeManifest(data, homeOverride) {
  const { configDir, manifestPath } = resolvePaths(homeOverride);
...
I could not run the 7 acceptance commands here due the same sandbox failure.  
If you want, I can re-run and confirm outputs once command execution is available.