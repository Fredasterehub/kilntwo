I couldn’t apply changes in the workspace because every read/write tool call is failing with `Sandbox(LandlockRestrict)`.  
Below is the complete content for `src/markers.js`:
...
I also couldn’t run validation commands because of the same sandbox failure.