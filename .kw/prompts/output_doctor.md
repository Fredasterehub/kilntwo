Local execution is blocked in this session (`Sandbox(LandlockRestrict)` on every tool call), so I could not actually write `src/doctor.js` or read `package.json` from disk.  
Here is the complete implementation matching your spec:
...
If you want, I can retry and apply it directly once the sandbox execution issue is cleared.